...
public class ScopeDataChart {
  ...
  private void calculateVariation() {
    final BigDecimal plannedValue = this.costDataChart.getPlannedValue();
    BigDecimal foreseenValue = this.costDataChart.getForeseenValue();

    final boolean hasPlannedValue = plannedValue != null
                                    && !this.isZero(plannedValue);
    ...
    if(Objects.equals(foreseenValue.intValue(), 0)) {
      foreseenValue = BigDecimal.ONE;
    }
  ...
  private BigDecimal calculateDeltaBetweenPlannedAndForeseen() {
    ...
    if(this.plannedWork == null) {
      ...
      final BigDecimal foreseenWork = Optional.ofNullable(this.foreseenWork).orElse(BigDecimal.ZERO);
