...
public class BaselineController implements IBaselineController {
  ...
  public Response<EntityDto> create(
    ...
    List<GetAllBaselinesResponse> projectBaselines = this.getAllBaselinesService
        .getAllByWorkpackId(request.getIdWorkpack());
    if (projectBaselines.stream().anyMatch((baseline) -> baseline.getStatus() == Status.PROPOSED)) {
      // Projeto já possui uma Linha de Base proposta, aguardando aprovação ou rejeição
