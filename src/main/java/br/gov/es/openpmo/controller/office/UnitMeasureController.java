...
public class UnitMeasureController {
  ...
  public ResponseEntity<ResponseBase<UnitMeasureDto>> findById(@PathVariable final Long id) {
    ...
    final ResponseBase<UnitMeasureDto> response = new ResponseBase<UnitMeasureDto>().setData(officeDto).setSuccess(true);
    return ResponseEntity.ok(response);
  }
  ...
  public ResponseEntity<ResponseBase<EntityDto>> update(@RequestBody final UnitMeasureUpdateDto unitMeasureUpdateDto) {
    final UnitMeasure unitMeasure = this.unitMeasureService.save(this.unitMeasureService.getUnitMeasure(unitMeasureUpdateDto));
    final ResponseBase<EntityDto> entity = new ResponseBase<EntityDto>().setMessage(OPERATION_SUCCESS).setData(new EntityDto(
        unitMeasure.getId()))
      .setSuccess(true);
    return ResponseEntity.ok(entity);
