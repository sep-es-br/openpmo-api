...
public interface StakeholderRepository extends Neo4jRepository<IsStakeholderIn, Long> {
    ...
    List<IsStakeholderIn> findByIdWorkpackAndIdActor(@Param("idWorkpack") Long idWorkpack, @Param("idActor") Long idActor);

    @Query("MATCH (p:Person)-[is:IS_STAKEHOLDER_IN]->(o:Workpack) WHERE id(o) = $idWorkpack AND (id(p) = $idPerson) RETURN p,o,is")
    List<IsStakeholderIn> findByIdWorkpackAndIdPerson(@Param("idWorkpack") Long idWorkpack, @Param("idPerson") Long idPerson);
