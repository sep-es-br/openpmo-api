...
public interface DashboardMilestoneRepository extends Neo4jRepository<Milestone, Long> {
  ...
  @Query("MATCH (w:Workpack{deleted:false, canceled:false})<-[:IS_IN*]-(m:Milestone{deleted:false , canceled:false})<-[:FEATURES]-(dm:Date) " +
          "WHERE id(w)=$workpackId " +
          "OPTIONAL MATCH (m)<-[:IS_SNAPSHOT_OF]-(s:Milestone{deleted:false , canceled:false })<-[:FEATURES]-(ds:Date) " +
          "OPTIONAL MATCH (s)-[:COMPOSES]->(b:Baseline{active: true }) " +
          "RETURN m.completed AS completed, dm.value AS milestoneDate, ds.value AS snapshotDate")
  ...
  @Query("MATCH (w:Workpack{deleted:false, canceled:false})<-[:IS_IN*]-(m:Milestone {deleted:false , canceled:false })<-[:FEATURES]-(dm:Date) " +
          ...
          "MATCH (m)<-[:IS_SNAPSHOT_OF]-(s:Milestone{deleted:false ,canceled:false })<-[:FEATURES]-(ds:Date) " +
          "RETURN m.completed AS completed, dm.value AS milestoneDate, ds.value AS snapshotDate")
