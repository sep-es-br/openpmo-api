...
public interface BaselineRepository extends Neo4jRepository<Baseline, Long>, CustomRepository {
  ...
  @Query(
    "MATCH (w:Workpack{deleted:false})-[s:IS_BASELINED_BY]->(b:Baseline{cancelation: true, status: 'PROPOSED'}) " +
  ...
  @Query(
    "MATCH (w:Workpack{deleted:false})-[s:IS_BASELINED_BY]->(b:Baseline{status: 'PROPOSED'}) " +
    "WHERE id(w)=$idWorkpack " +
    "RETURN count(b)>0"
  )
  ...
  @Query(
    "MATCH (workpack:Workpack{deleted:false})-[:IS_BASELINED_BY]->(baseline:Baseline{active: true,cancelation:false}) " +
  ...
  @Query(
    "MATCH (w:Workpack{deleted:false}) " +
  ...
  @Query(
    "MATCH (b:Baseline)<-[i:IS_BASELINED_BY]-(w:Workpack) " +
    "WHERE id(w)=$idWorkpack " +
    "RETURN b,i,w"
  )
  ...
  @Query(
    "MATCH (w:Workpack{deleted:false})<-[:IS_SNAPSHOT_OF]-(s:Workpack)-[:COMPOSES]->(b:Baseline) " +
    "WHERE id(w)=$idWorkpack AND (w.category <> 'SNAPSHOT' OR w.category IS NULL) AND id(b)=$idBaseline " +
    "RETURN count(s)>0"
  )
  boolean isSnapshotOfWorkpackComposingBaseline(Long idWorkpack, Long idBaseline);

  @Query(
    "MATCH (a:Workpack)-[:IS_SNAPSHOT_OF]->(w:Workpack{deleted:false})<-[:IS_SNAPSHOT_OF]-(s:Workpack)-[:COMPOSES]->" +
    "(b:Baseline) " +
    "WHERE id(a)=$idWorkpack AND (w.category <> 'SNAPSHOT' OR w.category IS NULL) AND id(b)=$idBaseline " +
    "RETURN count(s)>0"
  )
  boolean isSnapshotOfMasterComposingBaseline(Long idWorkpack, Long idBaseline);
  ...
  boolean hasStructureChanges(Long idChild, Long idBaseline);
  ...
  boolean hasBaselineStructureChanges(Long idChild, Long idBaseline);

  @Query(
    "MATCH (w:Workpack)<-[g1:IS_SNAPSHOT_OF]-(s:Workpack)-[c:COMPOSES]->(b:Baseline) " +
  ...
  @Query(
    "MATCH (w:Workpack{deleted:true})<-[:IS_SNAPSHOT_OF]-(:Workpack)-[:COMPOSES]->(b:Baseline) " +
  ...
  @Query(
    "MATCH (m:Workpack{deleted:false})<-[:IS_SNAPSHOT_OF]-(s:Workpack)-[:COMPOSES]->(b:Baseline) " +
    "WHERE id(m)=$idWorkpack AND id(b)=$idBaseline " +
    "RETURN count(s)>0"
  )
  boolean workpackHasSnapshot(Long idWorkpack, Long idBaseline);

  @Query(
    "MATCH (m:Workpack{deleted:false})<-[:IS_SNAPSHOT_OF]-(s:Workpack) " +
    "WHERE id(s)=$idWorkpack " +
    "RETURN count(s)>0"
  )
  boolean workpackHasMaster(Long idWorkpack);
  ...
  @Query(
    "MATCH (w:Workpack{deleted:false})-[:IS_IN]->(v:Workpack{deleted:false})<-[:IS_SNAPSHOT_OF]-(s:Workpack)-[:COMPOSES]->" +
    "(b:Baseline) " +
    "WHERE id(w)=$idChild AND id(b)=$idBaseline " +
    "RETURN s"
  )
  Optional<Workpack> findSnapshotOfParentByChildIdAndBaselineId(Long idChild, Long idBaseline);
  ...
  Optional<Workpack> findSnapshotByMasterIdAndBaselineId(Long idMaster, Long idBaseline);

  @Query(
    "MATCH (baseline:Baseline)<-[isBaselinedBy:IS_BASELINED_BY]-(workpack:Workpack{deleted:false}) " +
  ...
  @Query(
    "MATCH (b:Baseline)<-[c:COMPOSES]-(w:Workpack)-[iso:IS_SNAPSHOT_OF]->(m:Workpack)" +
  ...
  @Query(
    "MATCH (w:Workpack{deleted:true})<-[:IS_SNAPSHOT_OF]-(s:Workpack) " +
    "WHERE id(s)=$idWorkpack " +
    "RETURN count(w)>0"
  )
  ...
  @Query(
    "MATCH (master:Workpack{deleted:false}) " +
  ...
  @Query(
    "MATCH (b:Baseline)<-[:IS_BASELINED_BY]-(w:Workpack) " +
  ...
  @Query(
    "MATCH (b:Baseline), (w:Workpack{deleted:false}) " +
    "WHERE id(b)=$baselineId AND id(w)=$workpackId " +
    "CREATE (b)<-[:IS_BASELINED_BY]-(w)"
  )
  void createIsBaselinedByRelationship(@Param("baselineId") Long baselineId, @Param("workpackId") Long workpackId);

  @Query(
    "MATCH (b:Baseline), (s:Property) " +
  ...
  List<Baseline> findAllWaitingOthersEvaluationByPersonId(Long personId, String term, Double searchCutOffScore);
  ...
  List<BaselineWorkpackDto> findAllWorkpacksMasterById(final Long id);
  ...
  @Query(
    "MATCH (s:Step)-[:COMPOSES]->(schedule:Schedule)-[:FEATURES]->(d:Deliverable) " +
    "WHERE ID(d) IN $ids " +
  ...
  @Query(
    "MATCH (s:Step)<-[:IS_SNAPSHOT_OF]-(snapshotStep:Step)-[:COMPOSES]->(snapshot:Schedule)-[:IS_SNAPSHOT_OF]->(schedule:Schedule)-[:FEATURES]->(d:Deliverable) " +
  ...
  @Query(
    "MATCH (s:Schedule)-[f:FEATURES]->(w:Workpack) WHERE id(w) IN $idWorkpack " +
  ...
  @Query(
    "MATCH (b:Baseline)<-[:COMPOSES]-(snapshot:Schedule)-[:IS_SNAPSHOT_OF]->(master:Schedule)-[:FEATURES]->(w:Workpack) " +
  ...
  @Query(
    "MATCH (c:CostAccount)<-[co:CONSUMES]-(s:Step)-[cp:COMPOSES]->(sc:Schedule)-[:FEATURES]->(d:Deliverable) " +
    "WHERE id(d) IN $ids " +
  ...
  @Query(
    "MATCH (b:Baseline)<-[:COMPOSES]-(w:Workpack) " +
  ...
  @Query(
    "MATCH (project:Project {deleted:false}) " +
