...
public interface DashboardRepository extends Neo4jRepository<Dashboard, Long> {
        ...
        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(master:Deliverable{canceled: false, deleted:false})<-[:IS_SNAPSHOT_OF]-(w:Deliverable{deleted:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step) "
                        +
        ...
        @Query("MATCH \n" +
                        "    (plan:Plan) \n" +
                        "    <-[:BELONGS_TO]-(w:Deliverable{deleted:false,canceled:false}) \n" +
                        "    <-[:FEATURES]-(s:Schedule) \n" +
                        "    <-[:COMPOSES]-(st:Step) \n" +
                        ...
                        "    ID(w) AS idWorkpack, \n" +
                        "    ID(plan) as idPlan, \n" +
                        "    toString(SUM(toFloat(co.plannedCost))) AS foreseenCost\n")
        List<DashboardWorkpackDetailDto> findAllCost(List<Long> workpackIds, Long planId, Long baselineId);

        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(master:Deliverable{canceled: false, deleted:false})<-[:IS_SNAPSHOT_OF]-(w:Deliverable{deleted:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step)-[co:CONSUMES]->(ca:CostAccount) "
                        +
        ...
        List<DashboardWorkpackDetailDto> findAllCostBaseline(List<Long> ids, List<Long> workpackIds,
                        LocalDate yearMonth, Long planId);

        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(master:Deliverable)<-[:IS_SNAPSHOT_OF]-(w:Deliverable{deleted:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step)-[co:CONSUMES]->(ca:CostAccount) "
                        +
        ...
        List<DashboardWorkpackDetailDto> findAllEarnedValueBaseline(List<Long> ids, List<Long> workpackIds,
                        LocalDate yearMonth, Long planId);
        ...
        List<DashboardWorkpackDetailDto> findAllEarnedValueBaselineByTotalBaseline(Set<Long> snapshotIds,
                        List<Long> baselineIds, List<Long> workpackIds, LocalDate yearMonth, Long idPlan);

        @Query("MATCH (w:Workpack{deleted:false,canceled:false})-[:IS_BASELINED_BY]->(b:Baseline{status: 'APPROVED'}) WHERE b.status IN ['APPROVED', 'PROPOSED'] "
                        +
        ...
        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(w:Deliverable{deleted:false,canceled:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step)-[co:CONSUMES]->(ca:CostAccount) "
                        +
                        "WHERE (w.category <> 'SNAPSHOT' OR w.category IS NULL) AND st.periodFromStart IS NOT NULL  " +
                        "AND ($workpackIds IS NULL OR ID(w) IN $workpackIds) " +
                        "AND ($planId IS NULL OR ID(plan) = $planId) " +
        ...
        @Query("CALL { "
                        ...
                        + "toString(SUM((toFloat(totalBaseline) / toFloat(totalPlannedWork)) * toFloat(stm.actualWork))) AS earnedValue "
                        + "ORDER BY date")
        List<EarnedValueByStepDto> findAllEarnedValueByTotalBaseline(Set<Long> snapshotStepIds, List<Long> baselineIds,
                        List<Long> workpackIds, Long idPlan);

        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(master:Deliverable{canceled: false, deleted:false})<-[:IS_SNAPSHOT_OF]-(w:Deliverable{deleted:false, canceled:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step)-[co:CONSUMES]->(ca:CostAccount) "
                        +
                        ",(s)-[:COMPOSES]->(b:Baseline{status: 'APPROVED'})<-[:IS_BASELINED_BY]-(pj:Project)<-[:IS_IN*]-(master) "
                        +
                        "WHERE ID(b) IN $baselineIds " +
        ...
        @Query("MATCH (plan:Plan)<-[:BELONGS_TO]-(master:Deliverable{canceled: false, deleted:false})<-[:IS_SNAPSHOT_OF]-(w:Deliverable{deleted:false, canceled:false})<-[:FEATURES]-(s:Schedule)<-[:COMPOSES]-(st:Step) "
                        +
                        ",(s)-[:COMPOSES]->(b:Baseline{status: 'APPROVED'})<-[:IS_BASELINED_BY]-(pj:Project)<-[:IS_IN*]-(master) "
                        +
                        "WHERE ID(b) IN $baselineIds " +
