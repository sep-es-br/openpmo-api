...
public interface PlanPermissionRepository extends Neo4jRepository<CanAccessPlan, Long> {
  ...
  @Query(
    "MATCH (plan:Plan)<-[belongsTo:BELONGS_TO]-(workpack:Workpack)<-[permission:CAN_ACCESS_WORKPACK]-(person:Person) " +
  ...
  @Query(
    "MATCH (person:Person)-[permission:CAN_ACCESS_PLAN]->(plan:Plan) " +
    "WHERE id(plan) = $idPlan " +
    "AND id(person) = $idPerson " +
    "RETURN count(permission) > 0"
  )
