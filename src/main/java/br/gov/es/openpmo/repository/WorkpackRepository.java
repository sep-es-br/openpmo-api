...
public interface WorkpackRepository extends Neo4jRepository<Workpack, Long>, CustomRepository {
  ...
  @Query(
      "MATCH (workpack:Workpack) " +
          "OPTIONAL MATCH (workpack)<-[:IS_SNAPSHOT_OF]-(:Workpack)-[:COMPOSES]->(baseline:Baseline{active:true}) " +
          "WITH workpack, baseline " +
          "WHERE id(workpack)=$idWorkpack " +
          "RETURN count(baseline)>0 ")
  ...
  @Query(
    ...
    "OPTIONAL MATCH (master)<-[isIn:IS_IN*]-(milestone:Milestone) " +
    "WITH master, isIn, milestone " +
    "RETURN milestone"
  )
  Set<Milestone> findAllMilestones(Long idWorkpack);


  @Query("MATCH (project:Workpack) " +
         "WHERE id(project)=$idProject " +
         "OPTIONAL MATCH (project)<-[:IS_IN*]-(:Deliverable)<-[:FEATURES]-(schedule:Schedule) " +
         "OPTIONAL MATCH (project)<-[:IS_IN*]-(:Milestone)<-[:FEATURES]-(date:Date) " +
  ...
  Optional<DateIntervalQuery> findIntervalInSchedulesChildrenOf(@Param("idProject") Long idProject);
