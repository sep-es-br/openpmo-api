...
//         + "WITH w, ro, pl, wp, wm "
  ...
  @Query(" MATCH (u:Person) where id(u)=$idUsr " 
       + " with * " 
       + " MATCH (o:Office)<-[:IS_ADOPTED_BY]-(plan:Plan)<-[belongsTo:BELONGS_TO]-(pw:Workpack{deleted:false,canceled:false})  " 
       ...
       + "     AND id(w) <> id(pw)  " 
       + "  " 
       + " WITH u, o,plan,belongsTo,pw,wi,w,isa,model " 
       + " OPTIONAL MATCH (u)-[uo:CAN_ACCESS_OFFICE]->(o)  " 
       + " OPTIONAL MATCH (u)-[up:CAN_ACCESS_PLAN]->(plan)  " 
       + " OPTIONAL MATCH (u)-[upw:CAN_ACCESS_WORKPACK]->()-[:IS_IN*0..]->(w) " 
       + " OPTIONAL MATCH (u)-[uw:CAN_ACCESS_WORKPACK]->()<-[:IS_IN*0..]-(w) " 
       + "  " 
       + " WITH u,uo,up,upw,uw,plan,belongsTo,pw,wi,w,isa,model " 
       + " WHERE   u.administrator " 
       + "     OR  uo is not null " 
       + "     OR  up is not null " 
       + "     OR  upw is not null " 
       + "     OR  uw is not null " 
       + "  " 
       + " WITH plan,belongsTo,pw,wi,w,isa,model " 
