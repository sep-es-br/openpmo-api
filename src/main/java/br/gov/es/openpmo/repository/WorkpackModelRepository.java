...
public interface WorkpackModelRepository extends Neo4jRepository<WorkpackModel, Long> {
  ...
  void deleteRelationshipBetween(Long childrenId, Long parentId);

  @Query(
      "MATCH (target:WorkpackModel) " +
          ...
          "WITH target, children, targetProperty, childrenProperty, childrenGroup, childrenPropertyGroup, targetPropertyGroup, targetGroup " +
          "WHERE id(target)=$id " +
          "DETACH DELETE target, children, targetProperty, childrenProperty, childrenGroup, childrenPropertyGroup, targetPropertyGroup, targetGroup "
  )
  ...
  @Query("match (w:Workpack)-[i:IS_INSTANCE_BY]->(m:WorkpackModel) " +
      ...
      "return count(i)>0")
  boolean isWorkpackInstanceByModel(Long idWorkpack, Long idWorkpackModel);
  ...
  void createRelationshipByWorkpackIdAndModelId(Long workpackId, Long workpackModelId);

  @Query("match (w:WorkpackModel), (p:PropertyModel) " +
      "where id(w)=$workpackModelId and id(p)=$propertyModelId " +
      "create (w)<-[:FEATURES]-(p)")
  void createFeaturesRelationship(Long workpackModelId, Long propertyModelId);
