...
public interface ConsumesRepository extends Neo4jRepository<Consumes, Long> {
  ...
  List<Consumes> findAllByIdAndWorkpack(
    @Param("id") Long id,
  ...
  @Query("MATCH (ca:CostAccount)<-[c:CONSUMES]-(st:Step) " +
         ...
         "RETURN ca,c,st")
  Optional<Consumes> findByStepIdAndCostAccountId(
    Long idStep,
    Long idCostAccount
  );

  @Query("MATCH (master:Step)<-[:IS_SNAPSHOT_OF]-(snapshot:Step)-[:COMPOSES]->(baseline:Baseline) " +
  ...
  Set<Consumes> findAllSnapshotConsumesOfStepMaster(
    Long idBaseline,
    Long idStepMaster
  );
