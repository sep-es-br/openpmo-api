...
public interface WorkpackPermissionRepository extends Neo4jRepository<CanAccessWorkpack, Long> {
  @Query(
    "MATCH (person:Person)-[canAccess:CAN_ACCESS_WORKPACK]->(workpack:Workpack) "
  ...
  @Query(
    "MATCH (person:Person)-[canAccessWorkpack:CAN_ACCESS_WORKPACK]->(workpack:Workpack)  " +
  ...
  Set<CanAccessWorkpack> findInheritedPermission(Long workpackId, Long personId);

  @Query(
    "MATCH (person:Person)-[permission:CAN_ACCESS_WORKPACK]->(workpack:Workpack) " +
