...
@Component
public class GetActiveReportModels {

  private static final Logger log = LoggerFactory.getLogger(GetActiveReportModels.class);

  private final ReportDesignRepository reportDesignRepository;

  public GetActiveReportModels(ReportDesignRepository reportDesignRepository) {
    this.reportDesignRepository = reportDesignRepository;
  }

  public List<GetAllReportModelsResponse> execute(Long idPlanModel) {
    log.info(
      "Buscando ReportDesigns Ativos pelo ID do PlanModel {}.",
      idPlanModel
    );
  ...
  private List<ReportDesign> getReportDesigns(Long idPlanModel) {
    ...
    final List<ReportDesign> reportDesigns = reportDesignRepository.findActiveByPlanModelId(idPlanModel);
    log.debug(
  ...
  private List<GetAllReportModelsResponse> getReportModelsResponses(Collection<ReportDesign> reportDesigns) {
    log.debug(
      "Mapeando {} reportDesigns Ativos para DTO.",
      reportDesigns.size()
    );
