...
public class StepService {
    ...
    public void delete(final Long id) {
        ...
        final List<Step> steps = getScheduleSteps(schedule).stream()
    ...
    public Step update(final StepParamDto stepParamDto) {
        ...
        if (!CollectionUtils.isEmpty(getConsumes(stepUpdate))) {
            final Set<Consumes> consumesDelete = getConsumes(stepUpdate).stream()
                    .filter(consumes -> getConsumes(step) == null ||
                            getConsumes(step).stream().noneMatch(c -> isNonNun(c.getId()) && c.getId().equals(consumes.getId())))
        ...
        if (isNonNun(getPeriodFromStart(stepUpdate))) {
            ...
            if (isSameStartYear(stepUpdate, schedule)
                    && isSameStartMonth(stepUpdate, schedule)
                    && !getPeriodFromStart(stepUpdate).equals(getStart(schedule))) {
                setScheduleStart(schedule, stepUpdate);
                this.saveSchedule(schedule);
            }

            if (getPeriodFromStart(stepUpdate).getYear() == getEnd(schedule).getYear()
                    && getPeriodFromStart(stepUpdate).getMonthValue() == getEnd(schedule).getMonthValue()
                    && !getPeriodFromStart(stepUpdate).equals(getEnd(schedule))) {
